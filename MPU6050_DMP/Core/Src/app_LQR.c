...
void LQR_Control(void)
{
    ...
    // Calculer la vitesse linéaire (m/s)
    x_speed = (Encoder_Left + Encoder_Right) / 2.0f
              * PI * Diameter_67 / 1000.0f / 1560.0f * Control_Frequency;

    // Calculer la position (intégration)
    x_pose += x_speed / Control_Frequency;

    // Calculer l'angle d'inclinaison (rad) et vitesse angulaire (rad/s)
    angle_x = Angle_Balance / 180.0f * PI;
    gyro_x = (angle_x - last_angle) * Control_Frequency;
    last_angle = angle_x;

    // Calculer la vitesse de rotation (rad/s) et angle de rotation (rad)
    gyro_z = (Encoder_Right - Encoder_Left) / Wheel_spacing / 1000.0f
             * PI * Diameter_67 / 1000.0f / 1560.0f * Control_Frequency;
    angle_z += gyro_z / Control_Frequency;

    //  Définir les consignes selon Car_Target_Velocity et Car_Turn_Amplitude_speed
    if(Car_Target_Velocity > 0.5f)  // Avant
    {
    ...
    else if(Car_Target_Velocity < -0.5f)  // Arrière
    {
    ...
    else if(fabs(Car_Turn_Amplitude_speed) > 0.5f)  // Rotation sur place
    {
        Target_x_speed = 0;
        Target_gyro_z = Car_Turn_Amplitude_speed / 3.0f;
        angle_z = 0;
    ...
    // 8️⃣ Calculer les accélérations avec LQR
    L_accel = -(K1 * x_pose +
    ...
    // Appliquer PWM si pas d'anomalie
